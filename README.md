# オラオラッシュとは

オラオラで敵をブッ飛ばして飛距離を競うXRゲームです！
<!-- ![oraora技育展サムネ2](https://github.com/user-attachments/assets/0528b35b-512e-488f-a8fe-9ffd242dbd7f)
<img width="1397" height="786" alt="oraora動画サムネ３" src="https://github.com/user-attachments/assets/773917e5-397d-4b57-a5d3-14cd590e9c5d" />
<img width="1397" height="786" alt="oraoraDEMOサムネ" src="https://github.com/user-attachments/assets/0641d86b-cab6-4dc1-9a84-d1f92cdbd5b0" />
-->
## プレイ動画

[![デモ動画](https://github.com/user-attachments/assets/0641d86b-cab6-4dc1-9a84-d1f92cdbd5b0)](https://youtu.be/WoJFK-IbKgA?si=UvcJQT-Z6B_3rL8j)

## 紹介動画（約３分）

[![紹介動画](https://github.com/user-attachments/assets/773917e5-397d-4b57-a5d3-14cd590e9c5d)](https://www.youtube.com/embed/xRYd8dV40Ic?si=Gra-ID8Q_H_tP8Vl&start=8574)

# インストール方法

Meta Quest 3で遊べます。（2や3Sでも可）
オラオラッシュのインストール方法をわかりやすく手順を示して解説した記事を書きました。ぜひお読みください。  
https://qiita.com/mofurune/items/34601bcff58e491d8d55

# 遊び方

１．両コントローラーの中指トリガーを押すと、スタープラチナが出る。

２．右コントローラーの人差し指トリガーを押し、時を止める。

３．時が止まってる間にオラオラする（両コントローラーを前後に振る）。\
　※敵を直接殴る必要はないです。その場で空中を殴るとスタープラチナの拳が敵に飛んでいきます。

４．『トドメ』が表示されたら、自分の拳が『赤い拳マーク』に当たるように殴りぬける。

５．敵がブッ飛び、スコア（飛距離）が確定する。\
　※多くオラオラするほど遠くへブッ飛びます。

６．右コントローラーのBボタンを押すと、１．に戻る。ゲームを何度でも遊べるぞ！


ARモードのやり方：\
　時止め（２．）の前に左コントローラーのスティックを押し込むと、ARモードになります！\
　※時止め後にARモードにするとバグります。
